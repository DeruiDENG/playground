...
export function encrypt(text: string, key: number): string {
  ...
  return text
    .split('')
    .map(char => processChar(char, parsedKeys, true))
    .join('');
}
...
export function decrypt(text: string, key: number): string {
  ...
  return text
    .split('')
    .map(char => processChar(char, parsedKeys, false))
    .join('');
}

function parseEncryptionKey(key: number): EncryptionKeys {
  const keyDigits = `00${key.toString()}`
    .split('')
    .slice(-3)
...
function findKeyMap(char: string): KeyMap {
  return keyMaps.find(keyMap =>
    keyMap.chars.some(charInMap => charInMap === char)
  );
}

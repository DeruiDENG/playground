...
export function nextBigger(n: number): number {
  ...
  const decreasedDigitsIndex = digits.findIndex(
    (value, index) => index > 0 && value < digits[index - 1]
  );
  ...
  const nextHigherDigit = Math.min(
    ...digitsToChange.filter(digit => digit > decreaseDigit)
  );
  const remainingDigit = [
    decreaseDigit,
    ...remove(digitsToChange, nextHigherDigit),
  ];
  const result = [
    ...remainingDigit.sort((a, b) => b - a),
    nextHigherDigit,
    ...unchangedDigits,
  ];
  return Number(
